...
public class SpiralMatrix {
	...
	public static List<Integer> spiralOrder(int[][] matrix) {
		...
		// 上
		for (int i = column / 2; i < n - (column / 2); i++) {
		...
		// 右
		column++;
		for (int i = column; i < m - (row - 1); i++) {
			result.add(matrix[i][n - column]);
		}

		// 下
		for (int i = n - column; i > column / 2; i++) {
			result.add(matrix[m - row][i]);
		}
		row++;

		// 左
		for (int i = m - row; i < matrix.length; i++) {
